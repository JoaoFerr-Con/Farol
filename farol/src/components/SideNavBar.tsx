...
export const SideNavBar: React.FC<SideNavBarProps> = ({ currentScreen, onNavigateScreen, onSwitchPersona }) => {
  ...
  const menuItems: { id: string; label: string; icon: string; targetScreen?: ScreenType }[] = [
    { id: 'dashboard', label: 'Meu Jardim', icon: 'dashboard', targetScreen: 'dashboard' },
    { id: 'checkin', label: 'Check-in Psicossocial', icon: 'fact_check', targetScreen: 'checkin' },
    { id: 'health', label: 'Minha Saúde', icon: 'favorite', targetScreen: 'health' },
  ...
  return (
    <aside className="hidden lg:flex fixed left-0 top-0 h-screen w-60 bg-[#181C23] text-white flex-col p-4 z-30 shadow-xl border-r border-neutral-800">
      {/* Brand Header */}
      ...
      <div className="pt-4 border-t border-neutral-800 space-y-2">
        ...
        <button
          ...
          className="w-full flex items-center gap-3 text-[#B9A6E8] hover:text-white px-3 py-2 rounded-xl text-xs font-semibold transition-colors hover:bg-neutral-800"
        >
