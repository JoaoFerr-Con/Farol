...
function getInitialScreen(): ScreenType {
  if (typeof window === 'undefined') return 'welcome' as ScreenType;
  return window.localStorage.getItem(ONBOARDED_KEY) ? 'dashboard' : 'welcome';
}
...
export default function App() {
  ...
  const handleNavigateScreen = (screen: ScreenType) => {
    ...
    // Ao chegar no Jardim pela primeira vez (pós check-in), marca onboarding concluído.
    if (screen === 'dashboard' && typeof window !== 'undefined') {
  ...
  const renderScreen = () => {
    switch (currentScreen) {
      case 'welcome':
        return <WelcomeScreen onNavigateScreen={handleNavigateScreen} />;
      case 'consent':
        return (
          <ConsentScreen
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FA] text-[#1C1B1C] flex antialiased selection:bg-[#E9DDFF] selection:text-[#210F49]">
      ...
      <div className={`flex-1 flex flex-col min-w-0 ${isDesktop ? 'lg:ml-60' : ''}`}>
        <TopHeader onNavigateScreen={handleNavigateScreen} onSwitchPersona={isDesktop ? undefined : handleSwitchPersona} />
