...
export const DashboardScreen: React.FC<DashboardScreenProps> = ({
  ...
  // a árvore nasce e evolui conforme o colaborador avança no guia.
  const TUTORIAL_STAGES = [
    { icon: '🌱', label: 'Plantio', scale: 0.35, title: 'Sua semente foi plantada', text: 'Esta é a sua Árvore da Saúde Mental. Ela nasce agora e cresce a cada prática de autocuidado que você completa.' },
    { icon: '🌿', label: 'Muda', scale: 0.6, title: 'Banco de Gotas', text: 'Toque para regar sua árvore! Cada gota representa minutos dedicados à sua saúde mental.' },
    { icon: '🌳', label: 'Árvore jovem', scale: 0.85, title: 'Atividades Recomendadas', text: 'Meditações guiadas, leituras e pausas ativas para acumular mais gotas e ver sua árvore crescer.' },
    { icon: '🌸', label: 'Em flor', scale: 1, title: 'Score Preditivo de Bem-estar', text: 'Métricas agregadas em total sigilo para acompanhar sua estabilidade emocional ao longo do tempo.' }
  ];
  ...
  const heroScale = inTutorial ? TUTORIAL_STAGES[tourStep!].scale : 1;
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-sm w-full shadow-2xl border border-neutral-200">
            ...
            <div className="flex items-center justify-between">
              ...
              >
                {tourStep === TUTORIAL_STAGES.length - 1 ? 'Concluir' : 'Próximo'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          >
            ...
            <div className="w-48 h-48 sm:w-52 sm:h-52 rounded-full bg-[#E5E2E2]/60 flex items-center justify-center overflow-hidden relative mb-6 shadow-sm border-4 border-white">
              ...
              {/* Semente — visível só na primeira fase do tutorial */}
              {inTutorial && tourStep === 0 && (
                <span className="text-5xl relative z-10 transition-opacity duration-500">🌱</span>
              )}

              {/* Tree Image — escala acompanha o passo do tutorial (ou fica 100% para usuários recorrentes) */}
              <img
                src={treeImgUrl}
                alt="Árvore do Jardim de Autocuidado"
                style={{ transform: `scale(${inTutorial && tourStep === 0 ? 0 : heroScale})` }}
                className={`w-full h-full object-cover absolute inset-0 mix-blend-multiply transition-transform duration-[900ms] ease-[cubic-bezier(0.34,1.56,0.64,1)] ${
                  inTutorial && tourStep === 0
                    ? 'opacity-0'
                    : `opacity-100 ${isTreeGrowing ? 'tree-grow' : isTreeSwayingIntense ? 'tree-sway intense' : 'tree-sway'}`
                }`}
              />
